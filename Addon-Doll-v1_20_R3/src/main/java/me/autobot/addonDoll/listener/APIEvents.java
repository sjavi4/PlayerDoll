...
public class APIEvents implements Listener {
    ...
    private void onSetConvertPlayer(SetConvertPlayerCheckProtocolEvent event) {
        event.setCheckProtocol(
                (listener) -> listener != null &&
                ((ServerLoginPacketListenerImpl)listener).connection.channel
                        .attr(Connection.ATTRIBUTE_CLIENTBOUND_PROTOCOL).get() == ClientIntent.LOGIN.protocol().codec(PacketFlow.CLIENTBOUND)
        );
    }
}
