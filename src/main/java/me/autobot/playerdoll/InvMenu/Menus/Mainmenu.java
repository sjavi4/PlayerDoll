...
public class Mainmenu extends InvInitializer {
    ...
    public void decorate(Player player,Player doll) {
        ...
        var map = LangFormatter.getDollConfig(stripedName).getConfigurationSection("Owner");
        ...
        ItemStack slot0 = ButtonSetter.setItem(Material.PLAYER_HEAD,null, ChatColor.GREEN+doll.getDisplayName(), Arrays.asList(
                ...
                LangFormatter.YAMLReplace("mainmenu.owner", '&', new Pair<>( "%a%", map.getString("Name") ), new Pair<>("%b%", map.getString("UUID"))),
        ...
        actionMap.put(slot0, () -> {
            ...
            player.sendMessage(LangFormatter.YAMLReplace("CopiedDollUUID",'&',new Pair<>( "%a%", doll.getUniqueId().toString())));
        });
        buttons.set(0,slot0);
        ...
        actionMap.put(slot7,()->{
            ...
            player.closeInventory();
            new me.autobot.playerdoll.newCommand.Execute.despawn(player, doll.getName(), null);
        });

        if (PlayerDoll.dollManagerMap.get(stripedName).getOwner().getName().equalsIgnoreCase(player.getName())) {
            ItemStack slot8 = ButtonSetter.setItem(Material.BARRIER, null, LangFormatter.YAMLReplace("mainmenu.remove", '&'), null);
            buttons.set(8, slot8);
            actionMap.put(slot8,()->{
                player.closeInventory();
                new me.autobot.playerdoll.newCommand.Execute.remove(player, doll.getName(),null);
            });
        }
