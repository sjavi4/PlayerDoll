...
public final class PlayerDoll extends JavaPlugin {
    ...
    private static final String configVersion = "0.0.5";
    ...
    public void prepareDollSpawn() {
        if (isFolia || !YAMLManager.getConfig("config").getBoolean("Global.DollJoinAtStart")) return;
            ...
            if (f != null) {
                ...
                if (yaml != null) {
                    ...
                    if (config.getBoolean("setting.Join At Start")) {
                        ...
                        File file = new File(PlayerDoll.getDollDirectory(), f);
                        YAMLManager.loadConfig(file, stripFileName, false);
