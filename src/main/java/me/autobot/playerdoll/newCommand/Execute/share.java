...
public class share extends SubCommand {
    ...
    public void execute() {
        ...
        if (shareList.contains(target.getUniqueId().toString())) {
            player.sendMessage(LangFormatter.YAMLReplace("PlayerAlreadyInShareList", '&', new Pair<>("%a%", target.getName()), new Pair<>("%b%", doll.getDollName())));
            return;
        }
        player.sendMessage(LangFormatter.YAMLReplace("AddShare", '&', new Pair<>("%a%", target.getName()), new Pair<>("%b%", doll.getDollName())));
