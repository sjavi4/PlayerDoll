...
public class spawn extends SubCommand {
    ...
    public spawn(CommandSender sender, Object doll, Object args) {
        ...
        var flag = YAMLManager.getConfig("flag");
        if (flag != null) {
            ...
            YAMLManager.reloadConfig(dollName);
        }
    ...
    public void execute() {
        try {
            ...
            if (PlayerDoll.isSpigot) doll = new SpigotDollImpl(serverPlayer.server, serverPlayer.serverLevel(), profile, serverPlayer);
            if (PlayerDoll.isPaperSeries) doll = new PaperDollImpl(serverPlayer.server, serverPlayer.serverLevel(), profile, serverPlayer);
            if (PlayerDoll.isFolia) doll = new FoliaDollImpl(serverPlayer.server, serverPlayer.serverLevel(), profile, serverPlayer);

            //AbstractDoll dolls = new AbstractDoll(serverPlayer.server,serverPlayer.serverLevel(),profile,serverPlayer);
