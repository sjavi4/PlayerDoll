    ...
    public boolean onCommand(CommandSender sender, Command command, String s, String[] args) {
        if (sender instanceof Player p) {
            ...
            for (int i = 10 * page; i < Math.min(10+10*page , filterlist.length); i++) {
                ...
                var config = YAMLManager.loadConfig(new File(PlayerDoll.getDollDirectory(), filterlist[i]),"*",false);
                var yaml = config.getConfig();
                var removed = yaml.getBoolean("Remove")? ChatColor.STRIKETHROUGH : "";
                TextComponent hoverText = new TextComponent( i+1 + ". " + color + removed + name);
