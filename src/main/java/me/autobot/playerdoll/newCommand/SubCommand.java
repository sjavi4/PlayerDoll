...
public abstract class SubCommand {
    ...
    public boolean checkPermission(CommandSender sender, String stripedDollName) {
        ...
        if (sender instanceof Player) {
            ...
            var config = YAMLManager.getConfig(stripedDollName);
            if (player.isOp()) {
                level = 3;
            } else if (config.getString("Owner.UUID").equalsIgnoreCase(player.getUniqueId().toString()) || config.getString("UUID").equalsIgnoreCase(doll.getStringUUID())) {
                level = 2;
