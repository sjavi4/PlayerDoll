    ...
    private void setEXP() {
        LiteralCommandNode<Object> exp = literal("exp")
                ...
                .then(builder.getDollTarget(player -> LangFormatter.YAMLReplace("cmd-hover.exp", player.getLevel()))
                        .executes(commandContext -> builder.performCommand(commandContext, player -> new Exp(player, 1)))
                        //.executes(commandContext -> performExp(commandContext, 1))
                        .then(literal("all").executes(commandContext -> builder.performCommand(commandContext, player -> new Exp(player, -1))))
                        //.then(literal("all").executes(commandContext -> performExp(commandContext, -1)))
                        .then(argument("levels", IntegerArgumentType.integer(1))
                                .executes(commandContext -> builder.performCommand(commandContext, player -> new Exp(player, IntegerArgumentType.getInteger(commandContext, "levels"))))))
