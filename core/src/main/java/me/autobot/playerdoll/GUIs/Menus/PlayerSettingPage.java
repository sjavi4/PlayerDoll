...
public class PlayerSettingPage extends DollInvHolder {
    ...
    public PlayerSettingPage(String doll, String target) {
        ...
        if (target == null) {
            // Gset
            inventory = Bukkit.createInventory(this, 54, LangFormatter.YAMLReplace("menuTitle.playerSetting",new Pair<>( "%a%","Everyone"),new Pair<>( "%b%", shortDollName)));
        } else {
            // Pset
            this.targetPlayer = Bukkit.getOfflinePlayer(target);
            inventory = Bukkit.createInventory(this, 54, LangFormatter.YAMLReplace("menuTitle.playerSetting",new Pair<>( "%a%",target),new Pair<>( "%b%", shortDollName)));
        }
        this.perm = PermissionManager.getInstance(dollConfig.getConfig().getString("Owner.Perm"));
        setupInventoryItem();
    }
    ...
    public void setupInventoryItem() {
        ...
        flag.forEach((s,o) -> {
            ...
            settingMap.put(m, (p, b) -> {
                if (!perm.flagPersonalDisplays.get(s) && !p.isOp()) {
                    return false;
                }
                if (doll == null) {
                    if (target == null) {
