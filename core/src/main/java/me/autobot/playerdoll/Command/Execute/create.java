...
public class create extends SubCommand {
    ...
    public create(CommandSender sender, Object doll, Object args) {
        ...
        if (!player.isOp() && !perm.canCreateDoll) {
        ...
        if (max != -1) {
            var dollCountMap = PlayerDoll.playerDollCountMap;
            String playerUUID = player.getUniqueId().toString();
            if (dollCountMap.containsKey(playerUUID)) {
                count = dollCountMap.get(playerUUID);
            } else {
                dollCountMap.put(playerUUID, 0);
            }
    ...
    public void execute() {
        ...
        dollData.set("Owner", new LinkedHashMap<String,String>(){{
            put("Name",player.getName());
            put("UUID",player.getUniqueId().toString());
            put("Perm",player.getPersistentDataContainer().get(PlayerDoll.permissionKey, PersistentDataType.STRING));
        }});
        ...
        PermissionManager perm = PermissionManager.getInstance(player);
        /*
        YamlConfiguration flag = ConfigManager.getPermission();
        if (flag != null) {
            Map<String,Object> configs = new LinkedHashMap<>(flag.getConfigurationSection("GlobalFlag").getValues(false));
            configs.keySet().forEach(k -> settings.put(k, flag.getBoolean("GlobalFlags."+k+".Default")));
        }

         */
        dollData.createSection("setting", perm.flagGlobalToggles);
