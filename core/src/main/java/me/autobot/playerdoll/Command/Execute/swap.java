...
public class swap extends SubCommand {
    ...
    public swap(CommandSender sender, Object doll, Object args) {
        ...
        if (!checkPermission(sender, dollName, "Swap")) return;
    ...
    public void execute() {
        ...
        if (!player.isOp() && intArgs.size() > 0) {
            ...
            intArgs.set(0,Math.max(perm.minSwapInterval,intArgs.get(0)));
        }
        EntityPlayerActionPack.Action action = ActionHandler.action(args[0].toLowerCase(), intArgs);
        doll.getActionPack().start(EntityPlayerActionPack.ActionType.SWAP_HANDS, action);
    }
}