...
public class jump extends SubCommand {
    ...
    String[] args;
    public jump() {}
    public jump(CommandSender sender, Object doll, Object args) {
        setPermission(MinPermission.Share, false);
        String dollName = checkDollName(doll);
        if (!checkPermission(sender, dollName, "Jump")) return;
    ...
    public void execute() {
        ...
        doll.getActionPack().start(actionPack.ActionType_jump(), action);
