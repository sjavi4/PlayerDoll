...
public class unshare extends SubCommand {
    ...
    public unshare(CommandSender sender, Object doll, Object args) {
        ...
        if (!checkPermission(sender, dollName, "Unshare")) return;
    ...
    public void execute() {
        ...
        if (!shareList.contains(target.getUniqueId().toString())) {
            player.sendMessage(LangFormatter.YAMLReplaceMessage("PlayerNotInShareList", '&', new Pair<>("%a%", target.getName()), new Pair<>("%b%", bDoll.getName().substring(1))));
            return;
        }
        player.sendMessage(LangFormatter.YAMLReplaceMessage("DelShare", '&', new Pair<>("%a%", target.getName()), new Pair<>("%b%", bDoll.getName().substring(1))));
