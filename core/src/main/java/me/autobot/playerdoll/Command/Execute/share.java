...
public class share extends SubCommand {
    Player player;
    IDoll doll;
    String[] args;
    String dollName;
    public share() {}
    public share(CommandSender sender, Object doll, Object args) {
        setPermission(MinPermission.Share, false);
        dollName = checkDollName(doll);
        if (!checkPermission(sender, dollName, "Share")) return;
    ...
    public void execute() {
        ...
        List<String> shareList = (List<String>) doll.getConfigManager().getDollSetting().get("Share");
        if (shareList.contains(target.getUniqueId().toString())) {
            player.sendMessage(LangFormatter.YAMLReplaceMessage("PlayerAlreadyInShareList", '&', new Pair<>("%a%", target.getName()), new Pair<>("%b%", bDoll.getName().substring(1))));
            return;
        }
        player.sendMessage(LangFormatter.YAMLReplaceMessage("AddShare", '&', new Pair<>("%a%", target.getName()), new Pair<>("%b%", bDoll.getName().substring(1))));
        shareList.add(target.getUniqueId().toString());
        doll.getConfigManager().getDollSetting().put("Share",shareList);
