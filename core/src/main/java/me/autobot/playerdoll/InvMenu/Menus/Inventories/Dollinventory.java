        ...
        if (event.getSlot() < 5 || event.getSlot() > 8) {
            ...
            actions.put(event.getSlot(), () -> {
                switch (event.getClick()) {
                    ...
                    case LEFT, RIGHT -> {
                        if (event.getSlot() >= 36) {
                            new me.autobot.playerdoll.Command.Execute.slot(event.getWhoClicked(),this.getDoll().getName(),new String[]{Integer.toString(event.getSlot()-35)});
                            ...
                            if (event.getClick() == ClickType.RIGHT && !event.getCurrentItem().getType().isBlock()) {
                                new me.autobot.playerdoll.Command.Execute.use(event.getWhoClicked(),this.getDoll().getName(),null);
                    ...
                    case DROP , CONTROL_DROP -> {
                        ...
                        new me.autobot.playerdoll.Command.Execute.drop(event.getWhoClicked(),this.getDoll().getName(),new String[]{slot,all});
