...
public class PlayerSettingmenu extends InvInitializer {
    ...
    public void decorate(Player player,Player doll) {
        ...
        Set<String> configKeys = flagFile.getConfigurationSection("PersonalFlags").getValues(false).keySet();
        Map<String, Object> toggleMap = new LinkedHashMap<>();
        configKeys.forEach(s -> toggleMap.put(s, flagFile.getBoolean("PersonalFlags."+s+".Default")));
        if (target == null) {
