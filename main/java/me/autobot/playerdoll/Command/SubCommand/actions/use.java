...
public class use implements SubCommandHandler {
    @Override
    public void perform(Player player, String dollName, String[] args) {
        ...
        doll.getActionPack().start(EntityPlayerActionPack.ActionType.USE,action);
            //case ("interval") -> Bukkit.getScheduler().runTaskTimer(PlayerDoll.getPlugin(), doll::use,0,tick);
            //case ("periodic") -> Bukkit.getScheduler().runTaskTimer(PlayerDoll.getPlugin(), doll::use,0,tick);
