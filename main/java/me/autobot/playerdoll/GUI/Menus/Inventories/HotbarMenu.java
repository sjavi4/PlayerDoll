...
public class HotbarMenu extends InventoryGUI {
    ...
    protected Inventory createInventory() {
        return Bukkit.createInventory(null, 9 , YAMLManager.getConfig("lang").getString("title.armor"));
    }
...
/*
    ...
    public void decorate(Player player) {
        ...
        ItemStack[] hotbar = Arrays.copyOfRange(doll.getInventory().getContents(),0,9);
        for (int i = 0; i<list.size();i++) {
            list.set(i,new Pair<>(hotbar[i],()->{}));
        }
        for (int i = 0 ; i< list.size();i++) {
