...
public class Inv extends SubCommand implements DollCommandExecutor {
    ...
    @Override
    public void execute() {
        sender.openInventory(DollGUIHolder.DOLL_GUI_HOLDERS.get(target.getUniqueId()).menus.get(DollGUIHolder.MenuType.BACKPACK).getInventory());
    }
    ...
    public int onCommand(CommandSender sender, CommandContext<Object> context) {
        ...
        if (!outputHasPerm(playerSender, DollConfig.getOnlineDollConfig(target.getUniqueId()), FlagConfig.PersonalFlagType.INV)) {
            return 0;
        }
